...
export default {
  ...
  name: "Update Issue",
  description:
    "Updates an issue. A transition may be applied and issue properties updated as part of the edit. [See the documentation](https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-issues/#api-rest-api-3-issue-issueidorkey-put)",
  version: "0.2.20",
  annotations: {
  ...
  props: {
    ...
    transitionId: {
      label: "Transition ID",
      description:
        "The ID of the issue transition. Retrieving options requires a static `issueIdOrKey`. Required when specifying a transition to undertake.",
      propDefinition: [
        common.props.app,
        "transition",
        ({
  ...
  async additionalProps() {
    const {
      cloudId, projectId, issueTypeId, issueIdOrKey,
    } = this;
    ...
    try {
      ...
      if (!issueTypeId) {
        throw new ConfigurationError(
          "Please enter `projectId` and `IssueTypeId` to retrieve additional props.",
        );
      }
      const {
        projects: [
  ...
  async run({ $ }) {
    ...
    $.export(
      "$summary",
      `Issue with ID(or key): ${this.issueIdOrKey} has been updated.`,
    );
